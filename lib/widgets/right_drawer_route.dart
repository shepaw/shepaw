...
class RightDrawerRoute<T> extends RawDialogRoute<T> {
  ...
  ///
  /// 这里**不能**退化成「进度 <10% 且无向左甩动即关」的微滑判断：push 抽屉
  /// 路由（先弹后填，见 chat_screen 的 `_openSessionDrawer`）会触发
  /// Navigator._cancelActivePointers，把在途手指变成 PointerCancelEvent
  /// （速度强制归零）。此时手指仍停在触发阈值之上（识别器接受手势时必然
  /// ≥ 阈值），按阈值规则继续打开 —— 若按进度判断，手势刚过阈值就被 push
  /// 打断的真实短滑会被误判为微滑而立即 pop，正是「闪一下又回去了」。
  /// 识别器接受手势本身就是微滑过滤（左滑不足阈值或非横向主导根本进不到
  /// 这里），无需二次判断。
  ///
  /// 代价：结算用的 [openDx] 永远是接受帧的位移，「过阈值后回滑取消打开」
  /// 不再可能（回滑的 move 送不到识别器）。要恢复得让抽屉不走路由。
  void settleGesture({
    double velocityDx = 0,
    double openDx = 0,
