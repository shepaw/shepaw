...
class _GroupSessionListContentState extends State<_GroupSessionListContent> {
  ...
  void initState() {
    ...
    // 复用跨路由缓存时只把「最新消息时间已变 / 编排仍在进行」的行标过期
    // （见 [_markChangedStale]）：原先无条件全量标过期，等于每次左滑都把
    // 所有会话的首条/最新/未读/编排重查一遍，耗时随聊天内容增长。首帧不闪
    // 占位由缓存本身保证，不需要靠全量重查换取新鲜度。
  ...
  /// 所有行重跑四个查询；内容未变的会话直接沿用缓存。编排快照是文件读、
  /// 不进消息表，因此仍在编排中（状态非 finished）的行一并标过期，避免
  /// 「第 N 轮 🔄」角标停在旧轮次。
  Future<void> _markChangedStale() async {
    ...
    for (final id in ids) {
      ...
      final live = times[id];
      final orchestrationStatus =
          entry.data?.$4?['status'] as String? ?? '';
      if (cached == null ||
          live == null ||
          !live.isAtSameMomentAs(cached) ||
          (orchestrationStatus.isNotEmpty &&
              orchestrationStatus != 'finished')) {
        entry.stale = true;
      }
    }
    _refreshStalePreviews();
  ...
  Widget _buildList() {
    return ListView.builder(
      ...
      prototypeItem: const _GroupSessionTilePrototype(),
...
/// 群会话行的行高原型，只给 [ListView.builder] 的 `prototypeItem` 量高度用
/// （等价于 Offstage：不绘制、不参与命中测试）。
///
/// 结构与 [_GroupSessionListContentState._buildGroupSessionTile] 保持一致
/// （40×40 leading + 单行标题 + 最多两行副标题），高度随字号/主题走。
class _GroupSessionTilePrototype extends StatelessWidget {
  const _GroupSessionTilePrototype();
