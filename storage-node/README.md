...
- **无头管理面** `/admin`：用量、回收站列表/还原/清空；本机鉴权（admin token 或 loopback）
- Noise/WS 配对码与导入授权审批 UI 仍为后续
...
- API：`GET /admin/api/stats`、`GET /admin/api/recycle`、`POST /admin/api/recycle/empty`、`POST /admin/api/recycle/restore`（`Authorization: Bearer <token>` 或 `X-Admin-Token`）。
