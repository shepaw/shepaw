...
- A restricted allowlist also trims the shepaw tool `namespace` enum; if only specific commands such as `store.write` are enabled, the `subcommand` enum is trimmed too. The execution gate still enforces it. Everyday store I/O (`store`, except `declare` and `write --file` from a host path) and `help` skip approval; other commands can be allowed for the rest of the session from the confirm dialog. Non-safe `os` tools always confirm. Remote ACP agents use `hub.cli.execute` under the same rules. Agent Hub engines use the local `shepaw` shim: this device's pouch stays on the Hub, everything else is forwarded to the paired App gate — not `hub.cli.execute`.
