...
| M3 快照与恢复 | 5.1 定期快照 + GFS + 5.3 恢复 + 5.4 换机导入授权 + 密码变更策略 | 本机每日快照自动生成（GFS 清理）；master 为本机时快照即落盘；**远端 master 送达依赖 M4 同步引擎**。路径 A（旧设备在场）经授权可读并导入；路径 B（master 侧镜像）依赖 M4。改密后新旧快照各用其钥 |
