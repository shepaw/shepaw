...
2. 核对端点 URL 与配对码/白名单（远端 ACP v2.1 没有共享 Token）/ 端口与白名单（Peer / 隧道）。
3. 在添加/编辑 Agent 界面点「**测试连接**」诊断。
4. 本地 Agent 离线 → 检查 `shepaw meta system.info`、Agent 状态图标（🟢在线 / ⚪离线 / 🟡连接中 / 🔴错误）。
5. 外网 Peer 连不上 → 确认配对页 **我的二维码** 里「外网连接」已开且显示「Channel 已连接」（不是设置页）；或 `shepaw meta system.info`。
6. 离线消息收不到 → 确认收件箱投递后 App 上线会自动拉取，多等几秒或重进会话。
7. 仍不行 → 看日志：**设置 → 系统日志 / 推理日志**。
...
- **Q：个人档案 / 通讯录 / 她的朋友圈在哪？** 档案：设置 → 个人档案（She 也可用 `context profile.*` 读写）。通讯录：侧栏 → 通讯录。她的朋友圈：本机 She 详情页（不在储物袋里）；只和「本人」设备换摘要。
- **Q：外网配对 / 隧道在哪？** 通讯录 `+` → 配对设备 → **我的二维码** →「外网连接」。设置页没有「隧道」项。
- **Q：能不能和配对的手机直接聊天？** 不能。当前版本没有设备对人会话；配对用来共享 Agent、同步储物袋、换朋友圈摘要。
- **Q：灵魂、记忆、档案有什么区别？** 档案是用户客观信息（设置或 `profile.write`）；灵魂是你的人设（`memory.write --key soul`）；长期记忆是相处事件；认知是你对用户的主观印象。见第 2.12 节。
