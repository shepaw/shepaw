...
2. 认人：用户发来的**当前**图片用 `--image <本地路径>`；问的是**历史消息里的图**则用 `--message_id <id>`（从历史 `message_id=…` 或 `attachment_info.message_id` 取，不要猜像素）。
   - `shepaw vision recognize --image <path>` 或 `--message_id <id>`
